...
{
  HASH_INDEX_T cap = CAPACITIES[mIndex_];
  double load = static_cast<double>(numItems_ + numDeleted_) / static_cast<double>(cap);
  ...
  if(loc == npos) {
    throw std::logic_error("No free location found in hash table");
  }
...
{
  size_t numCaps = sizeof(CAPACITIES) / sizeof(HASH_INDEX_T);
  if(mIndex_ + 1 >= numCaps) {
    throw std::logic_error("No more capacities for resize");
  }
  ...
  for(HASH_INDEX_T i = 0; i < oldCap; i++) {
    ...
    if(hi != nullptr) {
      if(!hi->deleted) {
        HASH_INDEX_T start = hash_(hi->item.first) % newCap;
        prober_.init(start, newCap, hi->item.first);
        ...
        if(loc == npos) {
          delete hi;
          throw std::logic_error("Rehash failed during resize");
        }
        table_[loc] = hi;
        numItems_++;
      }
